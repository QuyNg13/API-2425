...
//   try {
//     const response = await fetch(`${API_BASE}/nsapp-stations/v3/nearest?lat=${lat}&lng=${lng}`, {
//       headers: { "Ocp-Apim-Subscription-Key": NS_API_KEY, "Accept": "application/json" }
//     });
...
// // Route om de vertrektijden op te halen
// app.get('/departures', async (req, res) => {
//   const { uicCode } = req.query;
//   if (!uicCode) return res.status(400).json({ error: "Geef een uicCode op" });

//   try {
//     const response = await fetch(`${API_BASE}/reisinformatie-api/api/v2/departures?uicCode=${uicCode}`, {
//       headers: { "Ocp-Apim-Subscription-Key": NS_API_KEY, "Accept": "application/json" }
//     });
...
//     const data = await response.json();
//     const departures = data.payload.departures.map(dep => ({
//       direction: dep.direction,
//       time: dep.plannedDateTime.substring(11, 16), // Alleen tijd weergeven (HH:MM)
//       track: dep.plannedTrack || "Onbekend"
//     }));
